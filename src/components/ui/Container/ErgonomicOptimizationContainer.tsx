    ...
    <div className="group w-[450px] h-[630px] pb-5 overflow-hidden flex flex-col shadow-md rounded-lg dark:shadow-[#ffffff19] shadow-[#00000025] transition-all duration-500 hover:scale-[1.03] gap-y-5 border-slate-300 dark:border-slate-700 hover:border-pink-600 dark:hover:border-pink-500 border-[1px] cursor-poiner">
      ...
      <section className="mx-5 flex flex-col gap-y-5 justify-between h-full">
        <section className="flex flex-col gap-y-8">
          <div className="transition-all duration-200 flex flex-row items-center gap-x-2 font-[600] text-[23px] group-hover:text-pink-600 text-pink-900">
            <div className="group-hover:scale-[1.05] transition-all duration-200 p-3 rounded-xl bg-pink-200 text-pink-800">
              <Eye size={25} />
            </div>
            Quantum Desk Optimization
          </div>
          <p className="text-slate-500 dark:text-slate-500 font-[400] text-lg">
            ...
            {["Neural monitor positioning", "Biometric chair adjustment"].map(
              (tile, index) => {
                return (
                  <div
                    key={index}
                    className="flex flex-row items-center gap-x-2 text-pink-700 font-[500] text-sm"
                  >
                    <div className="rounded-full bg-pink-600 w-3 h-3"></div>
                    <h5>{tile}</h5>
                  </div>
                );
              }
        ...
        <button className="flex items-center justify-center gap-x-2 border-none outline-none bg-gradient-to-r from-pink-600 to-red-500 w-full text-white rounded-lg py-2">
          <Star size={18} />
          Start Protocol
        </button>
      </section>
